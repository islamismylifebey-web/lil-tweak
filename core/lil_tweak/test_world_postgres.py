...
_WORLD_COLUMNS = (
    "id,owner_id,name,objective,repository_url,source_commit,checks,max_attempts,"
    "status,extract(epoch from created_at),extract(epoch from updated_at)"
)
_ATTEMPT_COLUMNS = (
    "id,world_id,owner_id,attempt_number,status,outcome,previous_attempt_id,feedback,"
    "cumulative_patch,plan,summary,tests,model_calls,input_tokens,output_tokens,total_tokens,"
    "extract(epoch from created_at),extract(epoch from started_at),extract(epoch from finished_at)"
)
...
def _attempt_from_row(row: Sequence[Any]) -> TestWorldAttempt:
    feedback_value = _json_value(row[7]) or []
    return TestWorldAttempt(
        ...
        outcome=None if row[5] is None else str(row[5]),
        previous_attempt_id=None if row[6] is None else str(row[6]),
        feedback=tuple(dict(item) for item in feedback_value),
        cumulative_patch=str(row[8] or ""),
        plan=str(row[9] or ""),
        summary=str(row[10] or ""),
        tests=str(row[11] or ""),
        model_calls=int(row[12] or 0),
        input_tokens=int(row[13] or 0),
        output_tokens=int(row[14] or 0),
        total_tokens=int(row[15] or 0),
        created_at=float(row[16]),
        started_at=None if row[17] is None else float(row[17]),
        finished_at=None if row[18] is None else float(row[18]),
    )
...
class PostgresTestWorldStore:
        ...
        *,
        ...
        with self._connect() as connection, connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                "VALUES (%s,%s,%s,%s,%s,%s,%s::jsonb,%s,'ready',%s,%s)",
                (
                    world_id,
                    owner_id,
                    name.strip(),
    ...
    def list_attempts(self, world_id: str, owner_id: str) -> list[TestWorldAttempt]:
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute(
                "SELECT 1 FROM lil_tweak_test_worlds WHERE id=%s AND owner_id=%s",
                (world_id, owner_id),
            )
    ...
    def enqueue_attempt(self, world_id: str, owner_id: str, *, idempotency_key: str) -> TestWorldAttempt:
        ...
        with self._connect() as connection, connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                "(id,world_id,owner_id,attempt_number,status,previous_attempt_id,attempt_idempotency_key) "
                "VALUES (%s,%s,%s,%s,'queued',%s,%s)",
                (attempt_id, world_id, owner_id, attempt_number, previous_attempt_id, idempotency_key),
            )
            cursor.execute(
    ...
    def reconcile_active_attempts(self, *, now: float | None = None, limit: int = 100) -> list[TestWorldAttempt]:
        ...
        with self._connect() as connection, connection.cursor() as cursor:
            ...
            for attempt_id, world_id, owner_id in recovered:
                cursor.execute(
                    "UPDATE lil_tweak_test_worlds SET status='running',updated_at=to_timestamp(%s) "
        ...
        *,
        ...
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute(
                ...
                "AND status='running' AND lease_expires_at>to_timestamp(%s) "
    ...
    def complete_attempt(
        ...
        *,
        ...
        with self._connect() as connection, connection.cursor() as cursor:
            cursor.execute(
                "SELECT world_id,owner_id,attempt_number FROM lil_tweak_test_world_attempts "
            ...
            cursor.execute(
                ...
                "output_tokens=%s,total_tokens=%s,finished_at=to_timestamp(%s),lease_owner=NULL,lease_expires_at=NULL "
        ...
        *,
        ...
        with self._connect() as connection, connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                "summary=%s,finished_at=to_timestamp(%s),lease_owner=NULL,lease_expires_at=NULL "
                "WHERE id=%s AND owner_id=%s AND lease_generation=%s",
                (
                    json.dumps(list(bounded_feedback), separators=(",", ":")),
                    summary,
