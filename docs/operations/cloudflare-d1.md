...
The private Sites source repository may have an independent history and newer owner-facing features than the public Core repository. Preserve that history and those features; do not force-push the Core tree over it or export private source/history into the public repository. Record the actual pushed `SITES_SOURCE_COMMIT` and `SITES_SOURCE_TREE` separately from the runtime's source identity. The native owner-flow receipt binds both Site identifiers, its actual archive/version/deployment and the runtime manifest. Only issue PASS after reviewing the compatible engineering integration and exercising the real owner flow across those exact releases.
