...
class Gate3RunnerConnectionVerifier:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            raise ValueError(
                "GALOR Runner V2 connection authorization minimum sequence is invalid"
            )
        if (
            ...
            raise ValueError(
                "GALOR Runner V2 connection authorization revocation epoch is invalid"
            )
        ...
        self._qualification_issuer_public_keys = dict(
            qualification_issuer_public_keys
        )
        self._qualification_runner_public_keys = dict(
        ...
        self._connection_authorization_signing_keys = dict(
            connection_authorization_signing_keys
        )
        self._connection_authorization_minimum_sequence = (
            connection_authorization_minimum_sequence
        )
    ...
    async def refresh(self) -> Gate3RunnerConnectionProof:
        ...
        async with self._refresh_lock:
            ...
            try:
                ...
                tenant_id, job_id, result = await (
                    LegacyRunnerConnectionVerifier._run_identity_heartbeat(
                        self,
                        handshake,
                    )
                )
                heartbeat = (
                    LegacyRunnerConnectionVerifier._validate_signed_heartbeat(
                        self,
                        result,
    ...
    def _validate_handshake(
        ...
        *,
        ...
        if (
            ...
            raise ExecutorUnavailableError(
                "GALOR Hub handshake freshness is invalid"
            )
        ...
        if authorization.authorization_id in self._seen_authorization_ids:
            raise ExecutorUnavailableError(
                "GALOR Runner V2 connection authorization was replayed"
            )
    ...
    def _verify_connection_authorization(
        ...
        *,
        ...
        signing_key = self._connection_authorization_signing_keys.get(
            cast(str, key_id)
        )
        ...
        if (
            ...
            raise ExecutorUnavailableError(
                "GALOR Runner V2 connection authorization id is invalid"
            )
...
def parse_secret_keys_json(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(
            f"GALOR Runner V2 {label} signing keys are invalid JSON"
        ) from exc
    if not isinstance(parsed, dict) or any(
        ...
        raise ValueError(
            f"GALOR Runner V2 {label} signing keys must be a string map"
        )
...
def _validate_secret_keys(keys: Mapping[str, str], label: str) -> None:
    if not keys or len(keys) > 16:
        raise ValueError(
            f"GALOR Runner V2 requires one to sixteen {label} signing keys"
        )
    for key_id, secret_value in keys.items():
        if not _is_safe_id(key_id):
            raise ValueError(
                f"GALOR Runner V2 {label} signing key identifier is invalid"
            )
        if (
            ...
            raise ValueError(
                f"GALOR Runner V2 {label} signing key secret is invalid"
            )
...
def _parse_iso(value: object, label: str) -> datetime:
    ...
    try:
        ...
        raise ExecutorUnavailableError(
            f"GALOR Runner V2 {label} is invalid"
        ) from exc
